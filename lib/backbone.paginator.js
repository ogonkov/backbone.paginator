...
(function (factory) {
  ...
  // Browser
    ...
    const oldPageableCollection = Backbone.PageableCollection;
    const PageableCollection = factory(_, Backbone);
  ...
  const _extend = _.extend;
  const _omit = _.omit;
  const _clone = _.clone;
  const _each = _.each;
  const _pick = _.pick;
  const _contains = _.includes;
  const _isEmpty = _.isEmpty;
  const _pairs = _.pairs || _.toPairs;
  const _invert = _.invert;
  ...
  function queryStringToParams (qs) {
    const params = {}, decode = decodeURIComponent;
    const kvps = qs.split("&");
    for (const param of kvps) {
      let [k, v] = param.split("=");
      if (v == null) v = true;
      k = decode(k);
      v = decode(v);
  ...
  function runOnceAtLastHandler (col, event, func) {
    const eventHandlers = col._events[event];
    if (eventHandlers && eventHandlers.length) {
      const lastHandler = eventHandlers[eventHandlers.length - 1];
      const oldCallback = lastHandler.callback;
  ...
  */
  const PageableCollection = Backbone.PageableCollection = Backbone.Collection.extend({
    ...
    */
    constructor: function (models, options) {
      ...
      const queryParams = _extend({}, PageableProto.queryParams, this.queryParams,
                                  options.queryParams || {});
      ...
      const state = this.state = _extend({}, PageableProto.state, this.state,
                                         options.state);
    ...
    */
    _makeFullCollection: function (models, options) {
      ...
      const properties = ["url", "model", "sync", "comparator"];
      ...
      const fullCollection = new (Backbone.Collection.extend(proto))(models, options);
    ...
    */
    _makeCollectionEventHandler: function (pageCol, fullCol) {
      ...
      return function collectionEventHandler (event, model, collection, options = {}) {
        ...
        const handlers = pageCol._handlers;
        ...
        const state = _clone(pageCol.state);
        let firstPage = state.firstPage;
        let currentPage = firstPage === 0 ?
          state.currentPage :
          state.currentPage - 1;
        const pageSize = state.pageSize;
        let pageStart = currentPage * pageSize, pageEnd = pageStart + pageSize;

        if (event == "add") {
          let pageIndex, fullIndex, addAt, colToAdd;
          ...
          if (colToAdd) {
            ...
            const modelToRemove = pageIndex >= pageSize ?
              model :
        ...
        if (event == "remove") {
          if (!options.onAdd) {
            ...
            if (!--state.totalRecords) {
              ...
              const totalPages = state.totalPages = ceil(state.totalRecords / pageSize);
            ...
            let nextModel;
            const removedIndex = options.index;
        ...
        if (event == "reset") {
          ...
          if (collection == pageCol && options.from == null &&
              options.to == null) {
            const head = fullCol.models.slice(0, pageStart);
            const tail = fullCol.models.slice(pageStart + pageCol.models.length);
        ...
        _each(_keys(handlers), function (event) {
          ...
          _each([pageCol, fullCol], function (col) {
            ...
            const callbacks = col._events[event] || [];
    ...
    */
    setPageSize: function (pageSize, options) {
      ...
      const totalPages = ceil(state.totalRecords / pageSize);
    ...
    */
    switchMode: function (mode, options) {
      ...
      const state = this.state = options.resetState ?
      ...
      const self = this;
      let fullCollection = this.fullCollection;
      const handlers = this._handlers = this._handlers || {};
      if (mode != "server" && !fullCollection) {
        fullCollection = this._makeFullCollection(options.models || [], options);
        fullCollection.pageableCollection = this;
        this.fullCollection = fullCollection;
        const allHandler = this._makeCollectionEventHandler(this, fullCollection);
      ...
      if (mode == "infinite") {
        ...
        const totalPages = ceil(state.totalRecords / state.pageSize);
        const lastPage = firstPage === 0 ? max(0, totalPages - 1) : totalPages || firstPage;
        for (let i = state.firstPage; i <= lastPage; i++) {
          links[i] = this.url;
        }
      }
    ...
    */
    getPage: function (index, options) {
      ...
      const mode = this.mode, fullCollection = this.fullCollection;
      ...
      const pageStart = (firstPage === 0 ? pageNum : pageNum - 1) * pageSize;
      const pageModels = fullCollection && fullCollection.length ?
    ...
    */
    getPageByOffset: function (offset, options) {
      ...
      let page = floor(offset / this.state.pageSize);
    ...
    sync: function (method, model, options) {
      ...
      if (self.mode == "infinite") {
        ...
        options.success = function (resp, status, xhr) {
          ...
          const newLinks = self.parseLinks(resp, _extend({xhr: xhr}, options));
    ...
    */
    parseLinks: function (resp, options) {
      const links = {};
      const linkHeader = options.xhr.getResponseHeader("Link");
      if (linkHeader) {
        const relations = ["first", "prev", "next"];
        _each(linkHeader.split(","), function (linkValue) {
          const linkParts = linkValue.split(";");
          const url = linkParts[0].replace(URL_TRIM_RE, "");
          const params = linkParts.slice(1);
          _each(params, function (param) {
            const paramParts = param.split("=");
            const key = paramParts[0].replace(PARAM_TRIM_RE, "");
            const value = paramParts[1].replace(PARAM_TRIM_RE, "");
    ...
    */
    parse: function (resp, options) {
      const newState = this.parseState(resp, _clone(this.queryParams), _clone(this.state), options);
      if (newState) {
    ...
    parseState: function (resp, queryParams, state, options) {
      if (resp && resp.length === 2 && _isObject(resp[0]) && _isArray(resp[1])) {
        ...
        const serverState = resp[0];

        _each(_pairs(_omit(queryParams, "directions")), function ([k, v]) {
          const serverVal = serverState[v];
    ...
    */
    fetch: function (options = {}) {
      ...
      const sortKey = _isFunction(this.queryParams.sortKey) ?
        this.queryParams.sortKey.call(this) :
        this.queryParams.sortKey;

      const order = _isFunction(this.queryParams.order) ?
      ...
      const extraKvps = _pairs(_omit(this.queryParams,
                                     _keys(PageableProto.queryParams)));
      for (let i = 0; i < extraKvps.length; i++) {
        const kvp = extraKvps[i];
        let v = kvp[1];
        v = _isFunction(v) ? v.call(this) : v;
      ...
      if (mode != "server") {
        const self = this, fullCol = this.fullCollection;
        const success = options.success;
    ...
    */
    _makeComparator: function (sortKey, order, sortValue) {
      ...
      return function (left, right) {
        let l = sortValue(left, sortKey), r = sortValue(right, sortKey), t;
    ...
    */
    setSorting: function (sortKey, order, options) {
      ...
      let delComp = false, delFullComp = false;
      ...
      const comparator = this._makeComparator(sortKey, order, options.sortValue);

      const full = options.full, side = options.side;
  ...
  const PageableProto = PageableCollection.prototype;
